...
Router.post("/new", passport.authenticate("jwt"), async (req, res) => {
    try {
      const newRestuarant = await restuarantModel.create(req.body.restuarantData);
      return res.json({ restaurants: newRestuarant });
    } catch (error) {
      return res.status(500).json({ error: error.message });
    }
  });
...
Router.patch("/update", passport.authenticate("jwt"), async (req, res) => {
    try {
      const updatedRestaurant = await restuarantModel.findByIdAndUpdate(
        req.body.restuarantData._id,
        { $set: req.body.restuarantData },
        { new: true }
      );

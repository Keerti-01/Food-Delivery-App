            ...
            <div className="hidden lg:flex gap-14 container px-20 mx-auto my-12">   
                ...
                { allTypes.map((items) => (
                    ...
                    <div className={classnames(
                            "flex items-center gap-3 p-2 transition duration-700 ease-in-out",
                             { "border-b-2 border-app-400" : type === items.id }  )}>

                        <div className={classnames(
                            "w-12 h-12 bg-gray-100 p-2 rounded-full",
                            { [`bg-${items.activeColor}-100`] : type === items.id } 
                            //{ "bg-app-50" : type === items.id }
                            ) } >

                            <img src={ type === items.id ? items.imageActive : items.imageDefault }
                            alt="delivery" className="w-full h-full "/>
                        </div>
                        <h3 className={ type === items.id ? "text-xl text-app-500" : "text-xl text-gray-600"}>
                            {items.name}</h3>
                    </div>
                    </Link>
                ))
